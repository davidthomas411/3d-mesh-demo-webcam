    ...
    def process_one_image(
        ...
        if bboxes is not None:
            ...
            if isinstance(detection_result, dict):
                ...
                # This demo tracks one subject. Keep the largest
                # detection before model inference so duplicate YOLO
                # detections cannot trigger a batch-size-2 compile.
                if len(boxes) > 1:
                    widths = np.maximum(
                        boxes[:, 2] - boxes[:, 0],
                        0.0,
